...
def dyn3d_3D_feed_imp(filename: str,iteration,nrows,ncols,nlayers):
    """
    ...
    os.chdir(path)
    x=np.array2string(x, 250, 3, ' ').replace("[", "")
    x=x.replace("]", "")
    output_file_feedback= open('ctf_feedback.dat','w')
    output_file_feedback.write(x)
